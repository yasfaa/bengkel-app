  ...
  <div v-if="modelValue" class="modal-backdrop" @click.self="$emit('update:modelValue', false)">
    <div class="modal-card" style="max-width: 520px;">
      ...
      <div v-if="selectedService" class="modal-body">
        <!-- Order Detail Brief -->
        <div style="background: #f8fafc; border: 1px solid var(--border-subtle); border-radius: 10px; padding: 14px; margin-bottom: 18px;">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
            <span style="font-size: 12px; font-weight: 600; color: var(--text-muted); text-transform: uppercase;">No. Polisi:</span>
            <strong class="nopol-font" style="color: var(--primary-color); font-size: 14px;">{{ selectedService.nopol }}</strong>
          </div>
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
            <span style="font-size: 12px; font-weight: 600; color: var(--text-muted); text-transform: uppercase;">Pelanggan:</span>
            <strong style="color: var(--text-main);">{{ selectedService.customerName }}</strong>
          </div>
          <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size: 12px; font-weight: 600; color: var(--text-muted); text-transform: uppercase;">Keluhan Awal:</span>
        ...
        <div style="background: #f8fafc; border: 1px solid var(--border-subtle); padding: 16px; border-radius: 10px; margin-top: 18px;">
          <div style="display: flex; justify-content: space-between; font-size: 13px; color: var(--text-secondary); margin-bottom: 8px;">
            ...
            <span class="numeric" style="font-weight: 600;">Rp {{ formatCurrency(selectedServiceMaster?.harga || 0) }}</span>
          </div>
          <div v-if="selectedSparepart" style="display: flex; justify-content: space-between; font-size: 13px; color: var(--text-secondary); margin-bottom: 8px;">
            <span>Sparepart ({{ selectedSparepart.name }}):</span>
            <span class="numeric" style="font-weight: 600;">Rp {{ formatCurrency(selectedSparepart.hargaJual) }}</span>
          </div>
          <div style="display: flex; justify-content: space-between; font-size: 16px; font-weight: 800; border-top: 1px solid var(--border-subtle); padding-top: 10px; margin-top: 10px; color: var(--text-main);">
            <span>Total Tagihan:</span>
            <span class="numeric" style="color: #059669;">Rp {{ formatCurrency(calculatedTotalInvoice) }}</span>
          </div>
        </div>
