<template>
  <div>
    ...
    <div class="card" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
      <div>
        <h3 style="font-size: 16px; font-weight: 800; color: var(--text-main);">Master Data & Kapasitas Teknisi Mekanik</h3>
        <p style="font-size: 13px; color: var(--text-muted); margin-top: 2px;">
          Kelola data teknisi, jadwal kerja, spesialisasi servis, dan status aktivitas pengerjaan unit
        </p>
      </div>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 20px;">
      <div v-for="mechanic in mechanics" :key="mechanic.id" class="card" style="display: flex; flex-direction: column; justify-content: space-between;">
        <div>
          <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 14px;">
            <div style="display: flex; gap: 12px; align-items: center;">
              <div style="width: 44px; height: 44px; border-radius: 10px; background: #eff6ff; display: flex; align-items: center; justify-content: center; color: var(--primary-color);">
                <i class="ph-bold ph-user-gear" style="font-size: 24px;"></i>
              </div>
              <div>
                <h4 style="font-size: 15px; font-weight: 800; color: var(--text-main);">{{ mechanic.nama }}</h4>
                <div style="font-size: 12px; color: var(--primary-color); font-weight: 600;">
                  {{ mechanic.spesialisasi || 'Umum & Tune Up' }}
                </div>
              </div>
            </div>
            <span :class="['badge', getMechanicStatus(mechanic.nama) === 'Bekerja' ? 'badge-working' : 'badge-done']">
              {{ getMechanicStatus(mechanic.nama) }}
            </span>
          </div>

          <div style="background: #f8fafc; border: 1px solid var(--border-subtle); border-radius: 8px; padding: 12px 14px; font-size: 13px; margin-bottom: 14px;">
            ...
            <div style="display: flex; justify-content: space-between;">
              ...
              <strong v-if="getMechanicActiveJob(mechanic.nama)" class="nopol-font" style="color: var(--primary-color);">
        ...
        <div style="display: flex; justify-content: flex-end; gap: 8px; border-top: 1px solid var(--border-subtle); padding-top: 12px; margin-top: 8px;">
          <button
