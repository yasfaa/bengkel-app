...
export function useTransactions(master = {}, queue = {}, ui = {}) {
  const showToast = ui.showToast || console.log;
  const { serviceMasters = ref([]), spareparts = ref([]) } = master;
  const { services = ref([]) } = queue;
  ...
  const selectedServiceMaster = computed(() => {
    if (!invoiceForm.value.serviceMasterId) {
      return serviceMasters.value.find((s) => s.is_active) || serviceMasters.value[0] || null;
    }
    return serviceMasters.value.find((s) => s.id === invoiceForm.value.serviceMasterId) || null;
  });

  const selectedSparepart = computed(() => {
    if (!invoiceForm.value.sparepartId) return null;
    return spareparts.value.find((p) => p.id === invoiceForm.value.sparepartId) || null;
  });
  ...
  const processPayment = async () => {
    ...
    try {
      ...
      if (selectedSparepart.value) {
        const part = spareparts.value.find((p) => p.id === selectedSparepart.value.id);
        if (part && part.stok > 0) {
          part.stok -= 1;
        }
      }
      ...
      const svc = services.value.find((s) => s.id === selectedService.value.id);
      if (svc) {
        svc.isPaid = true;
      }
      ...
      console.error(e);
      showToast('❌ Gagal memproses pembayaran invoice.', 3000);
    }
  };
  ...
  const saveStockIn = () => {
    if (!stockForm.value.sparepartId || stockForm.value.qty <= 0) {
      showToast('Pilih sparepart dan masukkan jumlah unit yang valid!', 3000);
      return;
    }

    const part = spareparts.value.find((p) => p.id === stockForm.value.sparepartId);
    if (part) {
