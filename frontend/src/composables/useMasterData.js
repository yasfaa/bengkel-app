...
export function useMasterData(ui = {}) {
  ...
  const deleteServiceMaster = async (service) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        console.error(e);
        showToast('❌ Gagal menghapus jasa servis.', 3000);
      }
    }
  };
  ...
  const saveServiceMaster = async () => {
    if (!serviceMasterForm.value.nama.trim() || serviceMasterForm.value.harga === null) {
      showToast('Nama dan harga jasa wajib diisi!', 3000);
      return;
    }
    ...
    try {
      ...
      console.error(e);
      showToast('❌ ' + (e.message || 'Gagal menyimpan jasa.'), 3000);
    }
  };
  ...
  const deleteMechanic = async (mechanic) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        console.error(e);
        showToast('❌ Gagal menghapus mekanik.', 3000);
      }
    }
  };
  ...
  const saveMechanic = async () => {
    if (!mechanicForm.value.nama.trim()) {
      showToast('Nama mekanik wajib diisi!', 3000);
      return;
    }
    ...
    try {
      ...
      console.error(e);
      showToast('❌ ' + (e.message || 'Gagal menyimpan mekanik.'), 3000);
    }
  };
  ...
  const saveSparepart = async () => {
    if (!sparepartForm.value.kode_part.trim() || !sparepartForm.value.nama.trim()) {
      showToast('Kode part (SKU) dan nama suku cadang wajib diisi!', 3000);
      return;
    }
    ...
    try {
      ...
      console.error(e);
      showToast('❌ ' + (e.message || 'Gagal menyimpan sparepart.'), 3000);
    }
  };
