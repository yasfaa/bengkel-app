...
export const useQueueStore = defineStore('queue', () => {
  ...
  watch(
    ...
    (newMasterId) => {
      ...
      const selected = masterStore.serviceMasters.find((s) => s.id === Number(newMasterId));
      if (selected) {
  ...
  watch(
    ...
    async (newBrandName) => {
      if (!newBrandName || !newBrandName.trim()) {
        masterStore.motorTypes = [];
        return;
      }
      const found = masterStore.motorBrands.find(
        (b) => b.nama.toLowerCase() === newBrandName.trim().toLowerCase()
      );
      if (found) {
        await masterStore.fetchMotorTypes(found.id);
      } else {
        masterStore.motorTypes = [];
      }
    }
  ...
  watch(
    ...
    async (newNopol) => {
      ...
      try {
        ...
        if (vehicle.brandId) {
          await masterStore.fetchMotorTypes(vehicle.brandId);
        }
  ...
  const saveNewService = async () => {
    ...
    try {
      ...
      uiStore.showToast('❌ Gagal menyimpan data PKB. ' + error.message, 4000);
    }
  };
  ...
  const confirmAssignMechanic = async ({
    ...
    try {
      ...
      console.error(e);
      uiStore.showToast('❌ Gagal mengalokasikan teknisi: ' + (e.message || ''), 4000);
    }
  };
