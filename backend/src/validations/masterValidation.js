...
const createServiceMasterSchema = z.object({
  nama: z.string({ required_error: 'Nama jasa servis wajib diisi.' }).trim().min(2, 'Nama jasa servis minimal 2 karakter.'),
  harga: z.coerce.number({ required_error: 'Harga jasa servis wajib diisi.' }).min(0, 'Harga tidak boleh bernilai negatif.'),
...
const createSparepartSchema = z.object({
  kode_part: z.string({ required_error: 'Kode part (SKU) wajib diisi.' }).trim().min(2, 'Kode part minimal 2 karakter.'),
  nama: z.string({ required_error: 'Nama suku cadang wajib diisi.' }).trim().min(2, 'Nama suku cadang minimal 2 karakter.'),
  kategori: z.string().trim().optional(),
...
const createSupplierSchema = z.object({
  nama: z.string({ required_error: 'Nama supplier wajib diisi.' }).trim().min(2, 'Nama supplier minimal 2 karakter.'),
...
const createBrandSchema = z.object({
  nama: z.string({ required_error: 'Nama merk motor wajib diisi.' }).trim().min(2, 'Nama merk minimal 2 karakter.'),
});
...
const createTypeSchema = z.object({
  nama: z.string({ required_error: 'Nama tipe motor wajib diisi.' }).trim().min(2, 'Nama tipe minimal 2 karakter.'),
...
const createCapacitySchema = z.object({
  kapasitas: z.string({ required_error: 'Kapasitas mesin wajib diisi.' }).trim().min(2, 'Kapasitas mesin minimal 2 karakter.'),
});
